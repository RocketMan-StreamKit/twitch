...
**Channel point rewards:** optional setting to delete Twitch rewards when they are removed from all triggers in the app (off by default). Generating a reward through a trigger reuses an existing reward with the same title and updates its cost. Saving a reward trigger without selecting or generating a reward is blocked. Optional emoji prefixes in generated reward titles (on by default): sounds — 🔊, overlays — 📺, hotkey integrations — ⌨️, game integrations — 🎮. If a reward used by triggers was deleted on Twitch, the addon recreates it automatically (using stored title/cost) and updates trigger bindings to the new reward id.
...
**Награды за баллы канала:** опциональная настройка удалять награды Twitch, когда они убраны из всех триггеров в приложении (по умолчанию выключено). При генерации награды через триггер используется существующая награда с таким же названием, а её стоимость обновляется. Сохранение триггера награды без выбора или генерации награды блокируется. Опциональные эмодзи в начале названия при генерации (по умолчанию включено): звуки — 🔊, оверлеи — 📺, хоткей-интеграции — ⌨️, игровые интеграции — 🎮. Если награда, используемая в триггерах, была удалена на Twitch, аддон автоматически пересоздаёт её (по сохранённым названию и стоимости) и обновляет привязки триггеров на новый id.
...
**Нагороди за бали каналу:** опціональне налаштування видаляти нагороди Twitch, коли їх прибрано з усіх тригерів у програмі (за замовчуванням вимкнено). Під час генерації нагороди через тригер використовується існуюча нагорода з такою ж назвою, а її вартість оновлюється. Збереження тригера нагороди без вибору або генерації нагороди блокується. Опціональні емодзі на початку назви під час генерації (за замовчуванням увімкнено): звуки — 🔊, оверлеї — 📺, хоткей-інтеграції — ⌨️, ігрові інтеграції — 🎮. Якщо нагороду, що використовується в тригерах, було видалено на Twitch, аддон автоматично перестворює її (за збереженими назвою та вартістю) і оновлює прив’язки тригерів на новий id.
